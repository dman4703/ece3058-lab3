...
void cachesim_init(int _block_size, int _cache_size, int _ways) {
    ...
    // hint: use the passed-in value (block size, cache size, ways) to calculate num_sets, num_offset_bits, num_index bits. utilize the simple_log_2 function
    // dynamically allocate cache sets: cache=(data type*)malloc(num_sets * sizeof(cache_set_t))
    // initialize each set- assign a LRU stack for each set (call the init_lru_stack())
